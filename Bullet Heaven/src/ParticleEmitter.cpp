...
ParticleEmitter::ParticleEmitter(sf::Vector2f npos, sf::Vector2f nbaseSpeed, sf::Vector2f nbaseSize, sf::Vector2f nremoveDistance, sf::Vector2f nspeedOffset, float nfadeTime, float nsizeVariation, float nbaseRotationSpeed, int nparticleCreateTimer, particleEmitterType ntype, sf::Color color):
pos(npos),
...
void ParticleEmitter::init(sf::Vector2f npos, sf::Vector2f nbaseSpeed, sf::Vector2f nbaseSize, sf::Vector2f nremoveDistance, sf::Vector2f nspeedOffset, float nfadeTime, float nsizeVariation, float nbaseRotationSpeed, int nparticleCreateTimer, particleEmitterType ntype, sf::Color ncolor){
	pos = npos;
	baseSpeed = nbaseSpeed;
	baseSize = nbaseSize;
	removeDistance = nremoveDistance;
...
void ParticleEmitter::createParticle(){
	...
	int tempSpeedXN = rand() % 2 + 0; // find if its gonna be negative or not
	int tempSpeedYN = rand() % 2 + 0;
	...
	if(tempSpeedXN == 0) // do the negatives
		tempSpeedX2 *= -1;
	if(tempSpeedYN == 0)
		tempSpeedY2 *= -1;
	...
	int tempSizeXN = rand() % 2 + 0;
	int tempSizeYN = rand() % 2 + 0;
