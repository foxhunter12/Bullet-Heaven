...
void Enemy::update(std::vector<std::unique_ptr<Bullet>>& playerBullets, sf::Vector2f playerPos){
		if(alive){
				shape.setPosition(pos);
				...
				bulletCreateTimer--;
				if(bulletCreateTimer <= 0){ // Creates bullet and decides the speed for the bullet so it can go towards the player.
